...
interface LogoProps {
  ...
  className?: string;
  href?: string | null;
  /** White-Logo-Variante für dunkle Hintergründe */
  inverted?: boolean;
}
 ...
 * KickPact Logo
 *
 * - `mark`: nur das K-Icon (SVG, currentColor)
 * - `full`: K-Mark + KICKPACT-Wordmark als PNG (exakte Brand-Typo)
 * - `tagline`: full + "Mehr als ein Spiel"
 */
export function Logo({ variant = "full", className, href = "/", inverted = false }: LogoProps) {
  const content =
    variant === "mark" ? (
      <MarkSvg className={cn("h-10 w-10", inverted ? "text-white" : "text-accent")} />
    ) : (
      <div className="flex items-center gap-2.5">
        <MarkSvg className={cn("h-9 w-9 shrink-0", inverted ? "text-white" : "text-accent")} />
        <div className="flex flex-col leading-none">
          ...
          {variant === "tagline" && (
            <span
              className={cn(
                "mt-1.5 text-[0.55rem] uppercase tracking-[0.2em] font-semibold",
  ...
  return (
    ...
    >
      <g transform="translate(0,1254) scale(0.1,-0.1)" fill="currentColor" stroke="none">
        ...
        <path d="M9220 5735 c-147 -33 -286 -101 -399 -195 -300 -252 -390 -679 -215 -1028 136 -270 395 -413 728 -399 272 11 487 124 649 342 344 460 146 1113 -383 1265 -98 28 -287 36 -380 15z"></path>
      </g>
    </svg>
  );
}
