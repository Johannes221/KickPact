...
async function seedSubscription(
  ...
  opts: {
    ...
    billingCycle?: "monthly" | "season_end";
...
describe("platform-stats", () => {
  ...
  it("getPlatformKpis counts active clubs and aggregates MRR", async () => {
    ...
    await seedTeamLicense(club1, team1, "pro"); // 899 monthly
    ...
    await seedTeamLicense(club2, team2, "basic"); // 499 monthly
    ...
    // 899 (pro) + 499 (basic) = 1398
    expect(k.mrrCents).toBe(1398);
  });

  it("getPlatformKpis counts season_end (Saison-Pass) subs into MRR", async () => {
    // Regression: PLAN_CYCLE_MONTHLY_CENTS keyed the cycle as `season` while the
    // DB billing_cycle enum yields `season_end` → Saison-Pass abos zählten mit 0 €.
    ...
    await seedTeamLicense(clubMonthly, teamMonthly, "pro"); // 899 monthly
  ...
  it("getVereinDetail returns full club info or null", async () => {
    ...
    await seedSubscription(clubA, { status: "active", billingCycle: "season_end" });
