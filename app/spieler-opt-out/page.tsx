 ...
 *
 * SECURITY/DSGVO (H2): Wieder aktiviert. Das Backend (opt-out-token,
 * inspectPlayerOptOutToken, confirmPlayerOptOut, players.blocked-Enforcement
 * im Crawler) ist vollständig funktionsfähig — nur diese Seite war zum Stub
 * degradiert, während die Datenschutzerklärung das Selbstbedienungs-Opt-out
 * weiterhin in Aussicht stellt (Art. 21 Widerspruch, betrifft auch
 * Minderjährige, deren Namen gescraped + öffentlich gezeigt werden).
 *
 * Aufruf: `/spieler-opt-out?token=<HMAC-Token>` (kein Login nötig)
 *
 * Flow:
 *   1. Token wird server-side inspiziert (zeigt Spielername + Team)
 *   2. User bestätigt → Server-Action `confirmPlayerOptOut`
