...
- Trigger-Konfiguration matched nicht: "Ich habe den Pledge nur für **Pflichtspiele** abgeschlossen, das war aber ein Freundschaftsspiel."
...
- Du verstehst die Trigger-Konfiguration falsch — z.B. Pledge gilt für Mannschaft generell, nicht nur deinen Sohn.

Wir sehen jedes Dispute. Sponsor mit hohem Dispute-Quote ohne sachliche Begründung kommt auf den Radar.
...
4. Sie können:
   - **Dispute akzeptieren** → Event wird gelöscht, keine Charge.
   - **Dispute ablehnen** → Event wird zu approved, Charge wird erzeugt.
...
- Bei keiner Reaktion: Event bleibt `disputed`, **keine Charge** entsteht. Verein verliert das Geld.
...
Du hast eine Charge schon vor Tagen bestätigt, jetzt fällt dir auf, dass es falsch war? Bis zum **7. des Folgemonats** kannst du noch anfechten:

1. **Sponsor-Dashboard → Abrechnungen → Aktueller Monat.**
2. Klick auf die Zeile, klick "..." → **"Charge anfechten"**.
3. Grund eingeben.
...
- **Pledge-Konfiguration deines eigenen Pledges** — wenn du den Pledge falsch konfiguriert hast, ist das deine Verantwortung, kein Dispute.
