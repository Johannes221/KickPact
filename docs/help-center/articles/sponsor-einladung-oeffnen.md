---
title: "Sponsor-Einladung öffnen"
slug: "sponsor-einladung-oeffnen"
category: "erste-schritte-sponsor"
category_label: "Erste Schritte — Sponsor"
prio: "MUSS"
audience: ["sponsor"]
related_articles:
  - "ersten-pledge-anlegen"
  - "sponsor-einladungslink-teilen"
  - "was-ist-ein-pledge"
last_updated: "2026-07-20"
status: "published"
---

Du hast einen Link von deinem Verein bekommen — vielleicht per WhatsApp, vielleicht per Mail. In zwei Minuten bist du als Sponsor angemeldet. Und nein, du brauchst nicht direkt deine Bankdaten.

## Was beim Klick passiert

Du landest auf einer Begrüßungsseite. Sie sagt dir:

- **Welche Mannschaft** dich eingeladen hat (mit Logo)
- Ggf. **dein Name**, wenn der Verein ihn beim Link-Erzeugen mitgegeben hat
- Eine kurze Erklärung, was als nächstes passiert
- Ein dicker oranger Button **"Pact anlegen →"**

Klick drauf. Wenn du noch keinen KickPact-Account hast, kommt jetzt der Login-Schritt. Wenn du schon eingeloggt bist, springst du direkt in den Pact-Wizard.

## Login-Schritt (Magic-Link)

KickPact arbeitet **ohne Passwörter** — du loggst dich mit einem Einmal-Link per Mail ein. So gehts:

1. E-Mail-Adresse eintragen.
2. KickPact schickt dir innerhalb von Sekunden einen Link.
3. Mail öffnen, Link klicken — du bist drin.

Du musst die Mail auf demselben Gerät öffnen, wo du gerade bist? **Nein, das ist nicht nötig.** Magic-Links funktionieren auch geräteübergreifend. Manche Sponsoren öffnen den Link am Handy, weil die Mail dort schneller ankommt — und werden dann auch dort eingeloggt.

## Sponsor-Profil ausfüllen

Beim allerersten Login richtest du ein Sponsor-Profil ein. KickPact fragt zwei Sachen:

1. **Anzeigename** — wie du als Sponsor heißen sollst. Dein Name ("Mehmet Yilmaz"), ein Familien-Alias ("Familie Schmidt") oder schlicht "Tante Erna".
2. **Dein Bezug zur Mannschaft** — z.B. "Papa von Tim", "Onkel von Lisa" oder einfach "Fan". Die Mannschaft sieht so, wer hinter ihr steht.

Tipp: Ein Konto pro Familie reicht. Oma, Opa und Patenonkel können sich einen Zugang teilen, die Zahlungsübersicht kommt gesammelt.

Das war's. Keine Bankdaten, keine SEPA-Lastschrift, keine Vorab-Authentifizierung. KickPact zieht **keine Beträge automatisch ein** — du zahlst, wenn die Zahlungsübersicht kommt, direkt an den Verein.

## Direkt im Pact-Wizard

Nach dem Profil bist du im Pact-Wizard. Hier legst du fest, **was du versprichst**:

- "5 € pro Tor der Mannschaft"
- "20 € wenn die Mannschaft gewinnt"
- "10 € pro Tor meines Sohnes Mehmet"

Mehr dazu in [Ersten Pact anlegen](ersten-pledge-anlegen.md) und [Was ist ein Pact?](was-ist-ein-pledge.md).

## Was du sehen kannst, sobald du drin bist

- **Live-Mannschaftsdaten** — Spielplan, letzte Ergebnisse, Tabelle (sofern auf Fußball.de gepflegt)
- **Dein aktueller Worst-Case** — was bist du monatlich maximal los, wenn die Mannschaft das Maximum trifft
- **Pending Events** — Ereignisse, die der Verein gemeldet hat und die du bestätigen kannst, siehe [Pending Events bestätigen](pending-events-bestaetigen.md)

## Wenn der Link nicht funktioniert

- **"Einladung ungültig oder abgelaufen"** → Links laufen nach 30 Tagen aus. Frag den Verein nach einem neuen.
- **"Einladung zurückgezogen"** → Der Verein hat den Link bewusst deaktiviert. Frag nach einem neuen.
- **Magic-Link-Mail kommt nicht an** → Spam-Ordner checken. Wenn auch dort nichts: Mail-Adresse falsch geschrieben? Du kannst es einfach nochmal versuchen, der alte Link bleibt 30 Tage gültig.

## Account-Sicherheit

Du hast kein Passwort — also auch keins zu verlieren. Jeder Login geht über einen Magic-Link an deine Mail. Schütze deinen Mail-Account, der Rest erledigt sich.

Weiter lesen:
- [Ersten Pact anlegen](ersten-pledge-anlegen.md)
- [Worst-Case verstehen](worst-case-berechnung-verstehen.md)
